...
class CanFrame {
    ...
    constructor(frameLength) {
        ...
        console.log("Frame created with Frame Length ", this.frameLength);
...
class GSUSBHostFrame {
    ...
    generate(frameDef, source, destination, timestamp_us) {
        ...
        if ( frameDef.fastPacket ) {
            ...
            const can_id = this.getCanId(frameDef.pgn, frameDef.priority, source, destination);
            let frame = 0;
            let offset = 0;
            ...
            data.setUint32(0, 0xFFFFFFFF, true);
            data.setUint32(4, this.getCanId(frameDef.pgn, frameDef.priority, source, destination), true);
            data.setUint8(8, 8, true); // dlc
            data.setUint8(9, 0, true); // channel
            data.setUint8(10, 0, true); // flags
            data.setUint8(11, 0, true); // reserved
    ...
    getCanId(pgn, priority, source, destination) {
        const canIdPF = (pgn >> 8) &0xff;
        if ( canIdPF < 240 ) { // PDU1 format
            if ( (pgn&0xff) != 0 ) {
                return 0;
            } else {
                return ((priority&0x7)<<26) 
                        | pgn<<8 
                        | destination<<8
                        | source;
            }
