...
const { GSUSBHostFrame } = require('./canframe.js');
...
class NMEA2000MessageDecoder {
    ...
    constructor() {
        ...
        this.gsUSBGenerator = new GSUSBHostFrame();
    }

    decode(frame) {
        frame.canFrame = CanRecorder.toString(Date.now()/1000, new Uint8Array(frame.toBuffer()));
        frame.canFrameLength = frame.frameLength;
        frame.fragment = false;
        frame.frameNo = 0;
        delete frame.fastPacketLength;
        delete frame.fastPacketRecievedLength;
        
        if ( frame.frameType !== "extended" ) {
            console.log("Not decoding frame, not extended type", frame);
            return message;
            ...
            if (NMEA2000MessageDecoder.messages[frame.messageHeader.pgn].fastPacket) {
                frame.fragment = true;
                const fastPacket = this.fastPacketHandler.handleFastPacket(frame);
                if ( fastPacket !== undefined ) {
                    // fast packet has been reconstructed, decode.
                    const message = NMEA2000MessageDecoder.messages[frame.messageHeader.pgn].fromMessage(fastPacket);
                    message.canPacket = CanRecorder.toString(Date.now()/1000, new Uint8Array(fastPacket.data.buffer));
                    message.canPacketLength = fastPacket.length;
                    message.fastPacket = true;
                    return message;
                }
            } else {

                // single packet message, frame is the message
                const message = NMEA2000MessageDecoder.messages[frame.messageHeader.pgn].fromMessage(frame);
                message.canPacket = CanRecorder.toString(Date.now()/1000, new Uint8Array(frame.data.buffer));
                message.canPacketLength = frame.data.byteLength;
                message.fastPacket = false;
                return message;
            }
    ...
    encode(pgnObj) {
        if ( NMEA2000MessageDecoder.messages[pgnObj.pgn] ) {
            const frameDef = NMEA2000MessageDecoder.messages[pgnObj.pgn].toMessage(pgnObj);
            return { 
                frameDef,
                frames : this.gsUSBGenerator.generate(frameDef, pgnObj.frame.messageHeader.sourceAddress,
                pgnObj.frame.messageHeader.destination,
                pgnObj.timestamp_us)
            };
        }
    }
...
class CANMessage {
    ...
    set2ByteUDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+1 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
            } else {
                const v = Math.round(value/factor);
                data.setUint16(byteOffset, v, true);
            }
        }
    }
    ...
    set2ByteDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+1 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0x7f);
            } else {
                const v = Math.round(value/factor);
                data.setInt16(byteOffset, v, true);
            }
        }
    }
    ...
    set8ByteUDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+3 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
                data.setUint8(byteOffset+2, 0xff);
                data.setUint8(byteOffset+3, 0xff);
                data.setUint8(byteOffset+4, 0xff);
                data.setUint8(byteOffset+5, 0xff);
                data.setUint8(byteOffset+6, 0xff);
                data.setUint8(byteOffset+7, 0x7f);
            } else {
                const v = Math.round(value/factor);
                data.setUint8(byteOffset, (v>>56)&0xff);
                data.setUint8(byteOffset+1, (v>>48)&0xff);
                data.setUint8(byteOffset+2, (v>>40)&0xff);
                data.setUint8(byteOffset+3, (v>>32)&0xff);
                data.setUint8(byteOffset+4, (v>>24)&0xff);
                data.setUint8(byteOffset+5, (v>>16)&0xff);
                data.setUint8(byteOffset+6, (v>>8)&0xff);
                data.setUint8(byteOffset+7, v&0xff);
            }
        }
    }
    ...
    set8ByteDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+3 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
    ...
    set4ByteUDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+3 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
                data.setUint8(byteOffset+2, 0xff);
                data.setUint8(byteOffset+3, 0x7f);
            } else {
                const v = Math.round(value/factor);
                data.setInt32(byteOffset, v, true);
            }
        }
    }
    ...
    set4ByteDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+3 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
    ...
    set3ByteDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+2 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
                data.setUint8(byteOffset+2, 0x7f);
            } else {

                const v = Math.abs(Math.round(value/factor));
                const sign = (value < 0)?0xff:0x00;
                data.setUint8(byteOffset, v&0xff);
                data.setUint8(byteOffset+1, (v>>8)&0xff);
                data.setUint8(byteOffset+2, (v>>16)&0x7f | sign);
            }
        }
    }
    ...
    set3ByteUDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+2 ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
                data.setUint8(byteOffset+1, 0xff);
                ...
                const v = Math.round(value/factor)&0xffffff;
                data.setUint8(byteOffset, v&0xff);
                data.setUint8(byteOffset+1, (v>>8)&0xff);
                data.setUint8(byteOffset+2, (v>>16)&0xff);
            }
        }
    }
    ...
    set1ByteUDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setUint8(byteOffset, 0xff);
            } else {
                data.setUint8(byteOffset, value/factor);
            }
        }
    }
    ...
    set1ByteDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset ) {
            if (value == CANMessage.n2kDoubleNA) {
                data.setInt8(byteOffset, 0x7f);
            } else {
                data.setInt8(byteOffset, value/factor);
            }
        }
    }
    ...
    set2ByteUInt(data, byteOffset, value) {
        if ( data.byteLength > byteOffset+1 ) {
    ...
    set2ByteInt(data, byteOffset, value) {
        if ( data.byteLength > byteOffset+1 ) {
    ...
    set1ByteInt(data, byteOffset, value) {
        if ( data.byteLength > byteOffset ) {
