...
class NMEA2000MessageDecoder {
    ...
    encode(pgnObj) {
        if ( NMEA2000MessageDecoder.messages[pgnObj.pgn] ) {
            const frameDef = NMEA2000MessageDecoder.messages[pgnObj.pgn](pgnObj);
            return this.gsUSBGenerator.generate(frameDef, pgnObj.messageHeader.sourceAddress,
                pgnObj.messageHeader.destination,
                pgnObj.timestamp_us);
        }
    }
}
    ...
    set8ByteUDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+3 ) {
            if (value == CANMessage.n2kDoubleNA) {
                ...
                const v = Math.round(value/factor);
                data.setBigUint64(byteOffset, v, true);
            }
        }
    }
    ...
    set8ByteDouble(data, byteOffset, value, factor) {
        if ( data.byteLength > byteOffset+3 ) {
            if (value == CANMessage.n2kDoubleNA) {
                ...
                const v = Math.round(value/factor);
                data.setBigInt64(byteOffset, v, true);
            }
        }
    }
