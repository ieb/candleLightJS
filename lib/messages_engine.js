...
class PGN127489_EngineDynamicParam extends CANMessage{
    ...
    toMessage(pgnObj) {
        if (pgnObj.pgn === 127489) {
            let status1 = 0x00;
            if ( pgnObj.status1.includes("Check Engine") ) status1 = status1 | 0x01;
            if ( pgnObj.status1.includes("Over Temperature") ) status1 = status1 | 0x02;
            if ( pgnObj.status1.includes("Low Oil Pressure") ) status1 = status1 | 0x04;
            if ( pgnObj.status1.includes("Low Oil Level") ) status1 = status1 | 0x08;
            if ( pgnObj.status1.includes("Low Fuel Pressure") ) status1 = status1 | 0x10;
            if ( pgnObj.status1.includes("Low System Voltage") ) status1 = status1 | 0x20;
            if ( pgnObj.status1.includes("Low Coolant Level") ) status1 = status1 | 0x40;
            if ( pgnObj.status1.includes("Water Flow") ) status1 = status1 | 0x80;
            if ( pgnObj.status1.includes("Water In Fuel") ) status1 = status1 | 0x100;
            if ( pgnObj.status1.includes("Charge Indicator") ) status1 = status1 | 0x200;
            if ( pgnObj.status1.includes("Preheat Indicator") ) status1 = status1 | 0x400;
            if ( pgnObj.status1.includes("High Boost Pressure") ) status1 = status1 | 0x800;
            if ( pgnObj.status1.includes("Rev Limit Exceeded") ) status1 = status1 | 0x1000;
            if ( pgnObj.status1.includes("EGR System") ) status1 = status1 | 0x2000;
            if ( pgnObj.status1.includes("Throttle Position Sensor") ) status1 = status1 | 0x4000;
            if ( pgnObj.status1.includes("Emergency Stop") ) status1 = status1 | 0x8000;
            let status2 = 0x00;
            if ( pgnObj.status2.includes("Warning Level 1") ) status2 = status2 | 0x1;
            if ( pgnObj.status2.includes("Warning Level 2") ) status2 = status2 | 0x2;
            if ( pgnObj.status2.includes("Power Reduction") ) status2 = status2 | 0x4;
            if ( pgnObj.status2.includes("Maintenance Needed") ) status2 = status2 | 0x8;
            if ( pgnObj.status2.includes("Engine Comm Error") ) status2 = status2 | 0x10;
            if ( pgnObj.status2.includes("Sub or Secondary Throttle") ) status2 = status2 | 0x20;
            if ( pgnObj.status2.includes("Neutral Start Protect") ) status2 = status2 | 0x40;
            if ( pgnObj.status2.includes("Engine Shutting Down") ) status2 = status2 | 0x80;
...
class PGN127505_FluidLevel extends CANMessage {
    ...
    toMessage(pgnObj) {
        if (pgnObj.pgn === 127505) {
            ...
            this.set2ByteUDouble(data, 1, pgnObj.actualTemperature, 0.01);
            this.set2ByteDouble(data, 3, pgnObj.fluidLevel, 0.001);
            this.set2ByteUDouble(data, 5, pgnObj.fluidCapacity, 0.01);
            this.setByte(data, 7, 0xff);
            return {
                priority: 6,
                type: "extended",
